...
func TestRequiredFlagNameCompletionInGo(t *testing.T) {
	...
	AssertErrNil(t, rootCmd.MarkFlagRequired("requiredFlag"))
	requiredFlag := rootCmd.Flags().Lookup("requiredFlag")

	rootCmd.PersistentFlags().IntP("requiredPersistent", "p", -1, "required persistent")
	AssertErrNil(t, rootCmd.MarkPersistentFlagRequired("requiredPersistent"))
	...
	AssertErrNil(t, childCmd.MarkFlagRequired("subRequired"))
...
func TestFlagFileExtFilterCompletionInGo(t *testing.T) {
	...
	AssertErrNil(t, rootCmd.MarkFlagFilename("file"))

	// Single extension
	rootCmd.Flags().StringP("log", "l", "", "log flag")
	AssertErrNil(t, rootCmd.MarkFlagFilename("log", "log"))

	// Multiple extensions
	rootCmd.Flags().StringP("yaml", "y", "", "yaml flag")
	AssertErrNil(t, rootCmd.MarkFlagFilename("yaml", "yaml", "yml"))

	// Directly using annotation
	rootCmd.Flags().StringP("text", "t", "", "text flag")
	AssertErrNil(t, rootCmd.Flags().SetAnnotation("text", BashCompFilenameExt, []string{"txt"}))
...
func TestFlagDirFilterCompletionInGo(t *testing.T) {
	...
	AssertErrNil(t, rootCmd.MarkFlagDirname("dir"))

	// Filter directories within a directory
	rootCmd.Flags().StringP("subdir", "s", "", "subdir")
	AssertErrNil(t, rootCmd.Flags().SetAnnotation("subdir", BashCompSubdirsInDir, []string{"themes"}))

	// Multiple directory specification get ignored
	rootCmd.Flags().StringP("manydir", "m", "", "manydir")
	AssertErrNil(t, rootCmd.Flags().SetAnnotation("manydir", BashCompSubdirsInDir, []string{"themes", "colors"}))
...
func TestValidArgsFuncCmdContext(t *testing.T) {
	...
	// We can safely use a basic type as key in tests.
	// nolint:staticcheck
...
// TODO: Enable or delete after checks.
...
// 	AssertErrNil(t, rootCmd.GenBashCompletion(buf))
// 	output := buf.String()
...
func TestCompleteCmdInZshScript(t *testing.T) {
	...
	buf := new(bytes.Buffer)
	AssertErrNil(t, rootCmd.GenZshCompletion(buf))
	output := buf.String()
...
func TestFlagCompletionInGo(t *testing.T) {
	...
	AssertErrNil(t, rootCmd.RegisterFlagCompletionFunc("introot", func(cmd *Command, args []string, toComplete string) ([]string, ShellCompDirective) {
		completions := []string{}
	...
	AssertErrNil(t, rootCmd.RegisterFlagCompletionFunc("filename", func(cmd *Command, args []string, toComplete string) ([]string, ShellCompDirective) {
		completions := []string{}
...
func TestFlagCompletionWorksRootCommandAddedAfterFlags(t *testing.T) {
	...
	// Important: This is a test for https://github.com/tprasadtp/pkg/cli/issues/1437
	// Only add the subcommand after RegisterFlagCompletionFunc was called,
...
// 	AssertErrNil(t, rootCmd.Execute())
// 	for _, cmd := range rootCmd.commands {
// 		if cmd.Name() == compCmdName {
...
func TestCompleteWithDisableFlagParsing(t *testing.T) {
	...
	// Test that when DisableFlagParsing==true,
	// ValidArgsFunction is called to complete flag names,
	// to complete the flags package knows about.
	...
	// not told of any flags, so it returns nothing
	expected = strings.Join([]string{
		"--persistent",
		"-p",
		"--nonPersistent",
