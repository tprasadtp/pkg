...
const asmTemplate = `// SPDX-FileCopyrightText: Copyright 2024 Prasad Tengse
// SPDX-License-Identifier: MIT
//
...
func generate(ctx context.Context) error {
	...
	// Ensure library path is known and is absolute.
	for _, item := range c {
		if item.LibraryPath == "" {
			...
			case "libc", "libSystem", "LibSystem":
				item.LibraryPath = libSystemPath
			default:
				return fmt.Errorf("unknown library: %w", err)
			}
		} else {
			if !filepath.IsAbs(item.LibraryPath) {
				return fmt.Errorf(
					"library path for %s(%s) must be absolute: %s",
					item.LibraryName, item.LibraryFunction, item.LibraryPath,
				)
			}
		}
	}

	err = render(c, "assembly", filepath.Join(output, "zz_generated.s"), asmTemplate)
	if err != nil {
		return err
	}
...
func render(c []*config, name string, out string, tpl string) error {
	...
	tasm, err := template.New(name).Parse(tpl)
	if err != nil {
