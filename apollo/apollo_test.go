...
func TestGoldenFileName(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			a := New(t, test.options...)
			assert.Equal(t, test.expected, a.GoldenFileName(t, test.name))
		})
	}
}
...
func TestEnsureDir(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if test.shouldExist {
				err := os.MkdirAll(target, a.dirPerms)
				assert.Nil(t, err)
			}

			if test.fileExist {
				err := os.MkdirAll(filepath.Dir(target), a.dirPerms)
				assert.Nil(t, err)
...
func TestUpdate(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			a := New(t)
			err := a.Update(t, test.name, test.data)
			assert.Equal(t, test.err, err)

			data, err := ioutil.ReadFile(a.GoldenFileName(t, test.name))
			assert.Nil(t, err)
	...
	*update = true
	...
	for i, tt := range firstTests {
		...
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			a.Assert(t, tt.filePrefix, sampleData)
		})
